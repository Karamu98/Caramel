...
bool ImGuiFileDialog::FileDialog(const char* vName, const char* vFilters, std::string vPath, std::string vDefaultFileName)
{
	...
	ImGui::Begin(vName, 0, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoCollapse);

	ImGuiViewport* test = ImGui::GetWindowViewport();
	ImVec2 windowSize = test->Size;
	ImGui::SetWindowSize(windowSize * 0.7f);
